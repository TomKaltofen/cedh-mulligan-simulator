...
def test_csv_file_exists(tmp_path: Path, sample_data: pl.DataFrame, sample_registry: CardRegistry) -> None:
...
def test_csv_has_expected_header(tmp_path: Path, sample_data: pl.DataFrame, sample_registry: CardRegistry) -> None:
...
def test_non_kept_rows_are_nan(tmp_path: Path, sample_data: pl.DataFrame, sample_registry: CardRegistry) -> None:
    ...
    df = _run_cooccurrence(sample_data, tmp_path, sample_registry)
    # Row 1 is mulligan
    assert df["CardCooccurrence"][1] is None
    # Rows 0, 2, 3 are kept
    assert df["CardCooccurrence"][0] is not None
    assert df["CardCooccurrence"][2] is not None
    assert df["CardCooccurrence"][3] is not None
...
def test_all_frequencies_in_range(tmp_path: Path, sample_data: pl.DataFrame, sample_registry: CardRegistry) -> None:
